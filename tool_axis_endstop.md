...
Full example configurations are in the `examples/axis_endstop/` directory:

* `printer.cfg` — shows the `toolchanger_x:x_virtual_endstop` endstop pin reference
* `toolhead_n.cfg` — shows `[tool_axis_endstop Tn]` with `x_pin` and `y_pin` per tool
* `homing.cfg` — shows homing configuration with per-tool X endstops
