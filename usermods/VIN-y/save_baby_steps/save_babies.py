    ...
    def save_babysteps(self, gcmd, babystep):
        ...
        if bbs != 0.0:
            try:
                ...
                self.gcode.respond_info("SAVE_BABYSTEPS: Config file not found at %s" % (printer_config,))
            except PermissionError:
                self.gcode.respond_info("SAVE_BABYSTEPS: Permission denied writing to %s" % (printer_config,))
            except Exception as e:
                self.gcode.respond_info("SAVE_BABYSTEPS: Error reading config: %s" % (str(e),))
